...
//GTYE4_COMMON clock connection
  ...
  //
  // DIFF_TERM must be FALSE on this board. The vendor export had "TRUE", which sets
  // DIFF_TERM_ADV on the port; that is only legal for I/O standards with internal
  // differential termination (e.g. LVDS). The 100 MHz clock here is DIFF_SSTL12 in
  // 1.2V bank 64, so "TRUE" fails DRC PORTPROP-6 before placement.
  IBUFGDS
   #(.DIFF_TERM("FALSE"))
   u_ibufgds
    (
